...
async def get_order_stats(
    ...
    custom_start: Optional[date] = Query(
        None, description="Custom start date (required for custom period)"
    ),
    custom_end: Optional[date] = Query(
        None, description="Custom end date (required for custom period)"
    ),
...
async def get_dashboard_stats(
    period: StatsPeriod = Query(
        StatsPeriod.LAST_7_DAYS, description="Period for time-based stats"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    try:
        ...
        dashboard_data = {
            ...
            "orders": {"period_data": order_stats.data, "summary": order_stats.summary},
            "revenue": {
        ...
        quick_stats = {
            ...
            "revenue": {
                ...
                "completion_rate": round(
                    (overview.completed_orders / overview.total_orders * 100), 2
                )
