    ...
    try:
        ...
        while current_date <= end_date:
            daily_stats[current_date] = OrderStatsDaily(
                date=current_date, package=0, food=0, laundry=0, total=0
            )
...
async def get_revenue_statistics(
    ...
    try:
        ...
        order_data = {
            row.order_date: (row.order_revenue or Decimal("0"), row.order_count)
            for row in order_result.all()
        }
        delivery_data = {
            row.delivery_date: row.delivery_revenue or Decimal("0")
...
async def get_user_statistics(
    ...
    try:
        ...
        active_users_data = {
            row.order_date: row.active_users for row in active_users_result.all()
        }
...
async def get_platform_overview(db: AsyncSession) -> PlatformOverview:
    ...
    try:
        ...
        total_customers_stmt = select(func.count(User.id)).where(
            User.user_type == UserType.CUSTOMER
        )
        total_riders_stmt = select(func.count(User.id)).where(
        ...
        # Active users today
        active_users_today_stmt = select(
            func.count(func.distinct(Order.owner_id))
    ...
    try:
        ...
        stmt = (
            ...
            .outerjoin(
                Review,
                and_(
                    Review.order_id == Order.id, Review.review_type == ReviewType.ORDER
                ),
            )
            .where(
                and_(
                    User.user_type.in_(
                        [UserType.RESTAURANT_VENDOR, UserType.LAUNDRY_VENDOR]
                    ),
                    Order.order_payment_status == PaymentStatus.PAID,
                )
            )
        ...
        redis_client.setex(
            cache_key,
            1800,
            json.dumps([v.model_dump() for v in top_vendors], default=str),
        )
...
async def get_comprehensive_stats(db: AsyncSession) -> ComprehensiveStatsResponse:
    ...
    try:
        ...
        order_status_stmt = select(
            Order.order_status, func.count(Order.id).label("count")
        ).group_by(Order.order_status)
        order_status_result = await db.execute(order_status_stmt)
        order_status_data = {
            row.order_status: row.count for row in order_status_result.all()
        }
        ...
        transaction_stats_stmt = select(
            ...
            .filter(Transaction.transaction_direction == TransactionDirection.CREDIT)
            .label("topups"),
            func.count(Transaction.id)
            .filter(Transaction.transaction_direction == TransactionDirection.DEBIT)
            .label("withdrawals"),
