...
async def create_new_rider(
    ...
    try:
        ...
        verification = VerificationCode(
            user_id=new_rider.id, expires_at=datetime.now() + timedelta(minutes=30)
        )
        db.add(verification)
...
async def recover_password(email: str, db: AsyncSession) -> dict:
    ...
    _html = send_password_request_email(
        user=user.email,
        expires_in="24 hours",
        custom_url=f"servipal://reset-password?token={reset_token}",
        reset_url=f"https://api.servi-pal.com/api/auth/reset-password?token={reset_token}",
...
async def generate_verification_codes(
    user: User, profile: Profile, db: AsyncSession
) -> tuple[str, str]:
...
async def verify_user_contact(
    ...
    verification_query = (
        ...
        .options(selectinload(VerificationCode.user).selectinload(User.profile))
        .where(
...
async def send_welcome_email(user: User):
    ...
    _html = send_welcome_email_template(
        title="Welcome to ServiPal",
        name=user.email.split("@")[0],
        body="Thank you for joining our platform. We're excited to have you!",
    )
...
async def register_user(
    ...
    try:
        ...
        # Create verification codes (auto-generated by database)
        verification = VerificationCode(
            user_id=user.id, expires_at=datetime.now() + timedelta(minutes=30)
        )
        db.add(verification)
        await db.flush()  # Trigger code generation
