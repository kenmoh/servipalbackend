...
async def _send_verification_codes(
    ...
    async with async_session as db:
        try:
            logger.info(f"Background task: Sending verification codes to user {user_id}")
            ...
            logger.info(f"Background task: Verification codes sent successfully to {email}")
            ...
            logger.error(
                f"Background task failed for user {user_id}: {str(e)}",
                exc_info=True
            )
...
async def register_user(db: AsyncSession, user_data: CreateUserSchema, background_tasks: BackgroundTasks) -> CreateUserResponseSchema:
    """
