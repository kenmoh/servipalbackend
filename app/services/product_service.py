...
from app.models.models import Item, User, Category
from app.schemas.product_schemas import ProductCreate, ProductUpdate, ProductResponse
from app.config.config import redis_client


async def create_product(
    db: AsyncSession, product_data: ProductCreate, seller: User
) -> ProductResponse:
    """
    ...
    try:
        ...
        set_cached_product(new_product.id, new_product.dict())
...
async def get_products(
    ...
    stmt = select(Item).offset(skip).limit(
        limit).order_by(Item.created_at.desc())
    result = await db.execute(stmt)
