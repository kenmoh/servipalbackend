...
async def handle_payment_webhook_old(request: Request, db: AsyncSession):
    payload = await request.json()
    event = payload.get("event")
    data = payload.get("data", {})
...
async def _process_successful_payment(db: AsyncSession, order: Order, payment_data: dict):
    ...
    if order.order_type == OrderType.PACKAGE:
        await producer.publish_message(
            service="wallet",
            operation="process_package_payment",
            payload={
                "order_id": str(order.id),
    ...
    await _dispatch_post_payment_tasks(order, PaymentStatus.PAID, db)
...
async def _dispatch_post_payment_tasks(order: Order, status: PaymentStatus, db: AsyncSession):
    ...
    # Notifications can be sent from here
    # Cache invalidation can also be triggered here
    logger.info(f"Order {order.id} processed with status: {status}. Kicking off post-payment tasks.")
    # Example: Invalidate cache
    redis_client.delete(f"order_details:{order.id}")
    redis_client.delete(f"user_orders:{order.owner_id}")


async def handle_payment_webhook(request: Request, db: AsyncSession):
    """
    Handles secure, server-to-server payment webhook from the payment provider.
    This is the single source of truth for processing payment events.
    """
    ...
    if event != "charge.completed":
        return {"status": "ignored", "reason": f"Ignoring event {event}"}

    tx_ref = data.get("tx_ref")
    if not tx_ref:
        logger.error("Webhook payload missing 'tx_ref'.")
        raise HTTPException(status_code=400, detail="Missing tx_ref")

    try:
        order_uuid = UUID(tx_ref)
    except ValueError:
        logger.error(f"Invalid UUID format for tx_ref: {tx_ref}")
        raise HTTPException(status_code=400, detail="Invalid tx_ref format")

    # Lock the order row to prevent race conditions
    order = await db.scalar(
        select(Order).where(Order.tx_ref == order_uuid).with_for_update()
    )
    ...
    # --- IDEMPOTENCY CHECK ---
    if order.order_payment_status == PaymentStatus.PAID:
        logger.warning(f"Order {order.id} is already paid. Ignoring duplicate webhook.")
        return {"status": "success", "reason": "Already processed"}

    # --- VERIFY WITH PROVIDER ---
    verification_data = await verify_transaction_tx_ref(tx_ref)
    if not verification_data or verification_data.get("status") != "success":
        logger.error(f"Transaction verification failed for tx_ref: {tx_ref}")
        await _process_failed_payment(db, order, PaymentStatus.FAILED)
        return {"status": "error", "reason": "Transaction verification failed"}

    verified_amount = verification_data.get("data", {}).get("amount")
    if Decimal(verified_amount) != order.grand_total:
        logger.error(
            f"Amount mismatch for tx_ref {tx_ref}. Expected {order.grand_total}, got {verified_amount}"
        )
        await _process_failed_payment(db, order, PaymentStatus.FAILED)
    ...
    try:
        if data.get("status") == "successful":
            await _process_successful_payment(db, order, data)
            return {"status": "success"}
        else:
            await _process_failed_payment(db, order, PaymentStatus.FAILED)
            return {"status": "success", "reason": "Processed failed status"}

    except Exception as e:
        logger.critical(f"CRITICAL: Failed to process payment for order {order.id} after verification: {e}", exc_info=True)
        # The transaction is verified but processing failed. This requires manual intervention.
        # Do NOT rollback the transaction here as the payment is already made.
        raise HTTPException(status_code=500, detail="Critical error during payment processing.")


async def order_payment_callback(request: Request, db: AsyncSession):
    """
    Simplified, user-facing callback endpoint.
    It only reads the order status (which is updated by the webhook) and shows a status page.
    ...
    if not tx_ref:
        return templates.TemplateResponse(
            "payment-status.html",
            {"request": request, "payment_status": "error", "message": "Transaction reference not found."},
        )
    ...
    try:
        ...
        return templates.TemplateResponse(
            "payment-status.html",
            {"request": request, "payment_status": "error", "message": "Invalid transaction reference."},
        )

    order = await db.scalar(select(Order).where(Order.tx_ref == order_uuid))

    if not order:
        return templates.TemplateResponse(
            "payment-status.html",
            {"request": request, "payment_status": "error", "message": "Order not found."},
        )

    # Display status based on what the webhook has updated
    return templates.TemplateResponse(
        "payment-status.html",
        {
            "request": request,
