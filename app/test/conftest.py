...
@pytest_asyncio.fixture
async def authenticated_dispatch_admin(
...
async def create_menu(
    async_client: AsyncSession, authenticated_restaurant_vendor: User
):
    headers = {"Authorization": f"Bearer {authenticated_user['access_token']}"}
    image_content = "This is the image"
    category = await create_category()
    files = [
        ("test.jpg", io.BytesIO(image_content), "image/jpeg"),
        ("test1.jpg", io.BytesIO(image_content), "image/jpeg"),
    ]
    menu_data = {
        "name": "Test Food Item",
    ...
    new_menu = await async_client.post(
        "/api/items/menu-item-create", data=menu_data, headers=headers, files=files
    )
