...
class TestUserCreation:
    ...
    async def test_create_rider(
        self, authenticated_dispatch_admin, async_client: AsyncClient
    ):
        """
        ...
        register_response = await async_client.post(
            f"{BASE_URL}/register-rider", json=rider_payload, headers=headers
        )
    ...
    async def test_create_user_invalid_email(
        self, async_client: AsyncClient, invalid_email
    ):
    ...
    async def test_login_user(self, async_client: AsyncClient):
        ...
        register_response = await async_client.post(
            f"{BASE_URL}/register", json=payload
        )
        assert register_response.status_code == 201
    ...
    @pytest.mark.parametrize(
        "invalid_field", ["email", "password", "user_type", "phone_number"]
    )
    async def test_create_user_missing_required_fields(
        self, async_client: AsyncClient, invalid_field
    ):
    ...
    async def test_create_user_with_existing_phone_number(
