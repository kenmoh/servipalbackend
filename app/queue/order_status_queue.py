...
class OrderStatusQueue:
    ...
    async def connect(self):
        ...
        if not self._connection:
            ...
            self._exchange = await self._channel.declare_exchange(
                "order_operations", ExchangeType.DIRECT, durable=True
            )
            ...
            self._dlq = await self._channel.declare_queue(
                "failed_order_updates", durable=True
            )
        ...
        try:
            ...
            async for db in get_db():
                try:
                    ...
                    await message.ack()
                    logger.info(
                        f"Successfully updated order {order_id} to status {new_status}"
                    )

                except Exception as db_error:
                    logger.error(
                        f"Database error processing order update: {str(db_error)}"
                    )
    ...
    async def publish_order_update(
        ...
        try:
            ...
            await self._exchange.publish(message, routing_key="order_update")
