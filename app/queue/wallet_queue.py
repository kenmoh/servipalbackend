...
class WalletQueue:
    ...
    async def connect(self):
        ...
        if not self._connection:
            ...
            self._exchange = await self._channel.declare_exchange(
                "wallet_operations", ExchangeType.DIRECT, durable=True
            )

            # Declare DLX first
            dlx = await self._channel.declare_exchange(
                "wallet_dlx", ExchangeType.DIRECT, durable=True
            )

            # Declare DLQ
            dlq = await self._channel.declare_queue(
                "failed_wallet_updates", durable=True
            )
    ...
    async def _update_transaction(
        self, db: AsyncSession, tx_ref: str, wallet_id: UUID, **kwargs
    ) -> Transaction:
        """Update existing transaction"""

        result = await db.execute(
            select(Transaction).where(
                Transaction.tx_ref == tx_ref, Transaction.wallet_id == wallet_id
            )
        )
            ...
            async for db in get_db():
                try:
                    async with db.begin():
                        ...
                        is_new_transaction = data.get("metadata", {}).get(
                        ...
                        if is_new_transaction:
                            ...
                            await self._create_transaction(
                                ...
                                tx_ref=data["tx_ref"],
                                amount=abs(
                                    Decimal(str(data.get("balance_change", 0)))
                                    or Decimal(str(data.get("escrow_change", 0)))
                                ),
    ...
    async def publish_wallet_update(self, **kwargs):
        ...
        try:
            ...
            headers = {
                ...
                "operation": (
                    "escrow_hold"
                    if Decimal(kwargs.get("escrow_change", 0)) > 0
                    else "escrow_release"
                    if Decimal(kwargs.get("escrow_change", 0)) < 0
                    else "wallet_update"
                ),
