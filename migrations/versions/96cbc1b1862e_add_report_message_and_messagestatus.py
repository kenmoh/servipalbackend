"""add Report, Message and MessageStatus

Revision ID: 96cbc1b1862e
Revises: c9c013341d90
Create Date: 2025-06-26 19:16:38.652627
...
revision: str = "96cbc1b1862e"
down_revision: Union[str, None] = "c9c013341d90"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "report_status",
            sa.Enum(
                "PENDING", "INVESTIGATING", "RESOLVED", "DISMISSED", name="issuestatus"
            ),
            nullable=False,
        ),
        ...
        sa.UniqueConstraint(
            "complainant_id", "delivery_id", name="uq_reporter_delivery_report"
        ),
        sa.UniqueConstraint(
            "complainant_id",
            "order_id",
...
def downgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "report_type",
            postgresql.ENUM("VENDOR", "CUSTOMER", "DISPATCH", name="reporttype"),
            autoincrement=False,
            nullable=False,
        ),
        sa.Column(
        ...
        sa.ForeignKeyConstraint(
            ["customer_id"], ["users.id"], name="issues_customer_id_fkey"
        ),
        sa.ForeignKeyConstraint(
            ["delivery_id"], ["deliveries.id"], name="issues_delivery_id_fkey"
        ),
        sa.ForeignKeyConstraint(
            ["dispatch_id"], ["users.id"], name="issues_dispatch_id_fkey"
        ),
        sa.ForeignKeyConstraint(
            ["order_id"], ["orders.id"], name="issues_order_id_fkey"
        ),
        sa.ForeignKeyConstraint(
            ["reporter_id"], ["users.id"], name="issues_reporter_id_fkey"
        ),
        sa.ForeignKeyConstraint(
            ["vendor_id"], ["users.id"], name="issues_vendor_id_fkey"
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["recipient_id"], ["users.id"], name="notifications_recipient_id_fkey"
        ),
        sa.ForeignKeyConstraint(
            ["report_issue_id"],
            ["issues.id"],
            name="notifications_report_issue_id_fkey",
        ),
        sa.ForeignKeyConstraint(
            ["sender_id"], ["users.id"], name="notifications_sender_id_fkey"
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["notification_id"],
            ["notifications.id"],
            name="notification_messages_notification_id_fkey",
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["sender_id"], ["users.id"], name="notification_messages_sender_id_fkey"
        ),
