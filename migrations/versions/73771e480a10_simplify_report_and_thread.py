"""simplify report and thread

Revision ID: 73771e480a10
Revises: 63fd7fe3772c
Create Date: 2025-06-27 18:17:33.158331
...
revision: str = '73771e480a10'
down_revision: Union[str, None] = '63fd7fe3772c'
...
def upgrade() -> None:
    ...
    # Use raw SQL to create enums only if they do not exist
    op.execute("""
    DO $$
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'reportedusertype') THEN
            CREATE TYPE reportedusertype AS ENUM ('VENDOR', 'DISPATCH', 'CUSTOMER');
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'reporttag') THEN
            CREATE TYPE reporttag AS ENUM ('COMPLAINANT', 'DEFENDANT');
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'reporttype') THEN
            CREATE TYPE reporttype AS ENUM ('DAMAGED_ITEMS', 'WRONG_ITEMS', 'LATE_DELIVERY', 'RIDER_BEHAVIOUR', 'CUSTOMER_BEHAVIOUR', 'OTHERS');
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'messagetype') THEN
            CREATE TYPE messagetype AS ENUM ('BROADCAST', 'REPORT');
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'usertype') THEN
            CREATE TYPE usertype AS ENUM ('CUSTOMER', 'DISPATCH', 'RESTAURANT_VENDOR', 'LAUNDRY_VENDOR', 'RIDER', 'ADMIN');
        END IF;
    END$$;
    """)
